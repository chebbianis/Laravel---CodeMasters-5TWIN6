...
@section('content')
<h1 style="margin-bottom: 1.5rem;">{{ isset($item) ? 'Modifier Item' : 'Créer un Item' }}</h1>
...
<form action="{{ isset($item) ? route('items.update', $item->id) : route('items.store') }}" 
    ...
    <button type="submit"
        ...
        {{ isset($item) ? 'Mettre à jour' : 'Créer' }}
    </button>
    <script>
document.querySelectorAll('#name, #description, #category_id, #condition, #status').forEach(field => {
    field.addEventListener('blur', () => {
        if (!field.checkValidity()) { // si le champ n'est pas valide
            field.focus();             // remet le focus sur ce champ
    ...
    // Optionnel : couleur en temps réel quand l'utilisateur tape
